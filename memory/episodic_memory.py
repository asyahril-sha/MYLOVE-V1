...
class AIEngineComplete:
    ...
    async def _update_state_from_context(self, context: Dict):
        """Update state tracker dari konteks dengan EPISODE RECORDING"""
        ...
        if context.get('location'):
            ...
            # Catat ke episodic kalau pindah
        ...
        if context.get('clothing'):
            ...
            # Update ke pakaian baru
            self.state.update_clothing(context['clothing'], reason)
            
            # Catat ke episodic kalau ganti baju
        ...
        if context.get('position'):
            ...
            # Ambil posisi lama
            old_position = self.state.current['position']['name']
            
            # Update ke posisi baru
            self.state.update_position(context['position'], desc)
            
            # Catat ke episodic kalau ganti posisi
            new_position = context.get('position')
            if old_position and new_position and old_position != new_position:
                await self.episodic.add_position_episode(
                    session_id=self.session_id,
        ...
        if context.get('activity'):
            # Ambil aktivitas lama
            old_activity = self.state.current['activity']['name']
            
            # Update ke aktivitas baru
            self.state.update_activity(context['activity'])
            
            # Catat ke episodic kalau ganti aktivitas
            new_activity = context.get('activity')
            if old_activity and new_activity and old_activity != new_activity:
                await self.episodic.add_activity_episode(
                    session_id=self.session_id,
                    from_act=old_activity,
                    to_act=new_activity
                )
        ...
        if context.get('mood'):
            # Ambil mood lama
            old_mood = self.state.current['mood']['primary']
            ...
            # Catat ke episodic kalau mood berubah
            new_mood = context.get('mood')
            if old_mood and new_mood and old_mood != new_mood:
                await self.episodic.add_mood_episode(
                    session_id=self.session_id,
                    from_mood=old_mood,
                    to_mood=new_mood,
                    reason=context.get('mood_reason', '')
        ...
        if context.get('arousal_delta'):
            ...
            # Ambil gairah lama
            old_arousal = self.state.current['arousal']['level']
            
            # Update gairah
            self.state.update_arousal(delta=delta, reason=reason)
            
            # Catat ke episodic kalau perubahan signifikan
            new_arousal = self.state.current['arousal']['level']
            if abs(new_arousal - old_arousal) >= 2:
                await self.episodic.add_arousal_episode(
                    session_id=self.session_id,
                    from_level=old_arousal,
                    to_level=new_arousal,
                    reason=reason
                )
            
            # Catat climax kalau mencapai puncak
            if delta >= 5 and new_arousal >= 9:
                await self.episodic.add_climax_episode(
                    session_id=self.session_id,
                    intensity=new_arousal,
